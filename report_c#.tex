...
\begin{verbatim}
# Deploy clusters (3 regions: europe-west1, us-west1, us-east1)
.\gcp\run_multi_region_experiments.ps1 -TargetClusterCount 3 -DaemonsPerRegion 3

# Run federated split experiment
.\gcp\run.ps1 -Run -Regions 3 -PointsPerRegion 3 -Concurrency 16
\end{verbatim}
...
\midrule
1 region, 1 point (baseline) & 33.6s & 1.0$\times$ \\
1 region, 3 points (3 IPs) & 23.1s & 1.5$\times$ \\
\textbf{3 regions, 9 points (9 IPs, federated)} & \textbf{8.8s} & \textbf{3.8$\times$} \\
\bottomrule
\end{tabular}
\end{table}
...
Wall-clock time is determined by the slowest region (8.8s), yielding the \textbf{3.8$\times$ speedup} over the single-point baseline. Total wall-clock time including overhead was 12.0s.
